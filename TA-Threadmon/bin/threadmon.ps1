...
$intro = @'
 ...
 |__   __| |                      | | |  \/  |           (_) |           (_)            
    | |  | |__  _ __ ___  __ _  __| | | \  / | ___  _ __  _| |_ ___  _ __ _ _ __   __ _ 
    | |  | '_ \| '__/ _ \/ _` |/ _` | | |\/| |/ _ \| '_ \| | __/ _ \| '__| | '_ \ / _` |
    | |  | | | | | |  __/ (_| | (_| | | |  | | (_) | | | | | || (_) | |  | | | | | (_| |
    |_|  |_| |_|_|  \___|\__,_|\__,_| |_|  |_|\___/|_| |_|_|\__\___/|_|  |_|_| |_|\__, |
                                                                                   __/ |
                                                                                  |___/ 
'@
    ...
    $ScriptBlock = {
        ...
        function local:Trace-Thread {
            ...
            if ($SystemInfo.ProcessorArchitecture -ne 0) {
                if (!$Kernel32::IsWow64Process($ProcessHandle, [ref]$Wow64)) { 
                    #Write-Error 'IsWow64Process failure.' 
                }
            }
            ...
            if ($Wow64)
            {
                ...
                if ($Kernel32::Wow64SuspendThread($hThread) -eq -1) { 
                    #Write-Error "Unable to suspend thread $ThreadId." 
                }
                if (!$Kernel32::Wow64GetThreadContext($hThread, $lpContextRecord)) { 
                   #Write-Error "Unable tof get context of thread $ThreadId." 
                }
            ...
            elseif ($SystemInfo.ProcessorArchitecture -eq 0)
            {
                ...
                if ($Kernel32::SuspendThread($hThread) -eq -1) { 
                    #Write-Error "Unable to suspend thread $ThreadId." 
                }
                if (!$Kernel32::GetThreadContext($hThread, $lpContextRecord)) { 
                    #Write-Error "Unable tof get context of thread $ThreadId." 
                }
            ...
            if ($Kernel32::ResumeThread($hThread) -eq -1) { 
                #Write-Error "Unable to resume thread $ThreadId." 
            }
            if (!$Kernel32::CloseHandle.Invoke($hThread)) { 
                #Write-Error "Unable to close handle for thread $ThreadId." 
            }
        }


        #write-Host "[*] Enumerating $($tname) threads of PID: $(Get-WmiObject -Class win32_service -Filter "name = '$tname'" | select -exp ProcessId)..." -ForegroundColor Yellow
        foreach ($Process in (Get-Process -Id (Get-WmiObject -Class win32_service -Filter "name = '$tname'" | select -exp ProcessId)))
            {
                ...
                if (!$Dbghelp::SymInitialize($ProcessHandle, $null, $false)) {
                    ...
                    if (!$Kernel32::CloseHandle.Invoke($ProcessHandle)) { 
                        #Write-Error "Unable to close handle for process $($Process.Id)." 
                    }
                    break
                }

                $Process.Threads | ForEach-Object -Process { Trace-Thread -ProcessHandle $ProcessHandle -ThreadId $_.Id -ProcessId $Process.Id -WaitReason $_.WaitReason}

                if (!$Dbghelp::SymCleanup($ProcessHandle)) { 
                    #Write-Error "Unable to cleanup symbol resources for process $($Process.Id)." 
                }
                if (!$Kernel32::CloseHandle.Invoke($ProcessHandle)) { 
                    #Write-Error "Unable to close handle for process $($Process.Id)." 
                }
    ...
    $ComputerIP = Get-NetIpaddress | Where addressstate -EQ preferred | where AddressFamily -EQ IPv4 | where InterfaceAlias -EQ Ethernet | %{$_.IPAddress}

    # loop through all services
    for($x=0; $x -lt $servicenames.Count;$x++){
        $threadParams = $servicenames[$x] -split ":"        
        $servicename = $threadParams[0]
        $threadfilter = $threadParams[1]

        $job = $(Get-WmiObject -Class win32_service -Filter "name = '$servicename'"-AsJob | Wait-Job -Timeout 20)
        if ($job.State -ne 'Completed') {
            $job | Get-Job | Remove-Job -Force
            return
        }
        $service = $job | Receive-Job
        if($service){
            if ($PSBoundParameters['ComputerName']) { $ReturnedObjects = Invoke-Command -ComputerName $ComputerName -ScriptBlock $RemoteScriptBlock -ArgumentList @($Name, $Id, $servicename) }
            else { $ReturnedObjects = Invoke-Command -ScriptBlock $ScriptBlock -ArgumentList @($Name, $Id, $servicename) }
     
            $threads = $ReturnedObjects | Where-Object {$_.MappedFile -like "*$threadfilter*"}

            $threadlist = ""
            if($threads) 
            {   

                [array]$ThreadIds = $threads    | %{$_.ThreadId}
                [array]$TProcessIds = $threads  | %{$_.ProcessId}
                [array]$TAddrPCs = $threads     | %{$_.AddrPC}
                [array]$TSymbols = $threads     | %{$_.Symbol}
                [array]$TMappedFiles = $threads | %{$_.MappedFile}
                [array]$TAddrReturns = $threads | %{$_.AddrReturn}
                [array]$tWaitReason = $threads | %{$_.WaitReason}

                #loop through threads of service
...
function Get-Uptime {
    ...
    $os = Get-WmiObject -Class win32_operatingsystem -Property LastBootUpTime
    $uptime = (Get-Date) - $os.ConvertToDateTime($os.LastBootUpTime)
    return $uptime
}

$totaluptime = Get-Uptime
$time = '{0:N0}' -f $totaluptime.TotalMinutes
$time = [int]::Parse($time)
if($time -ge 3){
    Threadmonitoring("EventLog:evt")
}